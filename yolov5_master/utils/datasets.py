...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        if cache_images:
            ...
            for i, x in pbar:
                self.imgs[i], self.img_hw0[i], self.img_hw[i] = x  # img_old, hw_original, hw_resized = load_image(self, i)
...
def load_image(self, index):
    # loads 1 image from dataset, returns img_old, original hw, resized hw
    img = self.imgs[index]
    if img is None:  # not cached
        ...
        return img, (h0, w0), img.shape[:2]  # img_old, hw_original, hw_resized
    else:
        return self.imgs[index], self.img_hw0[index], self.img_hw[index]  # img_old, hw_original, hw_resized
...
def hist_equalize(img, clahe=True, bgr=False):
    # Equalize histogram on BGR image 'img_old' with img_old.shape(n,m,3) and range 0-255
